        ...
        if event.to_me:
            ...
            if request == "start":
                ...
                if 'city_id' not in user_info.keys():
                    ...
                    if user_city == 'Я не нашел такой город, введи заново!':
                        write_msg(event.user_id,
                ...
                if 'bdate' not in user_info.keys():
                    write_msg(event.user_id,
                              "У тебя в профиле не указана дата рождения! Введи свою дату рождения в формате "
                              "день.месяц.год.(1.1.1900)")
